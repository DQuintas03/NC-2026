...
const areas = [
  { key: "acertos", label: "Acertos", icon: CheckCircle, color: "bg-sky-50 text-[#017cb7]" },
  { key: "trocas", label: "Trocas", icon: ArrowLeftRight, color: "bg-amber-50 text-amber-600" },
  { key: "faltas", label: "Faltas de Circulacao", icon: AlertTriangle, color: "bg-red-50 text-red-600" },
];
...
const AreaImportCard = ({ area, count, onUpload, onDelete, deleting }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="font-['Outfit'] font-semibold text-slate-900">{area.label}</h3>
          <p className="text-xs text-slate-500">{count} registos na base de dados</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {areas.map((area) => (
            <AreaImportCard
              key={area.key}
              area={area}
              count={stats ? stats[area.key] : 0}
              onUpload={() => setUploadArea(area.key)}
              onDelete={() => handleDelete(area.key)}
              deleting={deleting === area.key}
            />
          ))}
        </div>
